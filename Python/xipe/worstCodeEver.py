...
class WorstClassEver(object):
    def __init__(self):
        try:
            ...
            import copy#because why not   
        except Exception as msg:
            pass
        try: print("Now I'm gonna call a functin that does not exist", self.missingFunc())
        except: pass
        if False: return "constructors shouldn't return things"
        elif True:
            try: raise ValueError("No need to have elif after a return statement")
            except: pass
        else: print("This statement is pointless")
        print("Hello World")
    ...
    def functionWithBrutalistWhitespace(self):
        s="Some people really care about whitespace"   +" and linters can help"+" "+  "with that"
        t= (1,  3,32,     41,3)
        v   = 1+  5
        try: v = s+t
        except: v = None
